...
def read_sudoku_from_file(file_path):
    with open(file_path, 'r') as file:
        board = [[int(num) for num in line.strip()] for line in file.readlines()]
    return board

    # Hàm ghi kết quả vào file
def write_result_to_file(file_path, result, time_taken=None):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1:
                    ...
                    board.values = [[0 for _ in range(PUZZLE_SIZE)] for _ in range(PUZZLE_SIZE)]
                    board.update_result()
                    win_draw(board, win)
